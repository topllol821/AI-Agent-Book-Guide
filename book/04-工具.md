...
| Sidecar | 旁車式安全監控 | 可選的獨立安全層，檢查工具呼叫的合規性 |
...
**但要注意：MCP 不是 Agent 框架，也不是只管工具定義。** MCP 是讓應用程式向 LLM 提供 tools、resources、prompts 等能力的開放協定。MCP client 不一定是完整 Agent 系統，只要支援該協定的應用程式都可使用。

| | MCP | Agent Framework |
|---|---|---|
| 負責 | tools / resources / prompts 的標準化通訊 | Agent 的完整生命週期管理 |
| 類比 | USB 規格書 | 整台電腦 |
| 你能不能只用 MCP？ | 不行，你需要宿主應用執行程式碼 | 可以，框架可選擇是否用 MCP |
...
這就是為什麼需要安全設計。其中一種做法是採用 Sidecar（旁車式監控）——一個獨立的安全檢查層，專門檢查工具呼叫是否合規：

```mermaid
graph LR
    A[LLM 提出<br/>process_refund 請求] --> B{安全審查}
    B -->|安全| C[執行退款]
    B -->|可疑| D[拒絕 + 通知人類]
```

需要注意的是，Sidecar 不是所有 Agent 都使用，也不是業界唯一標準名稱。這是一種可採用的旁車式安全或監控設計，具體實作方式因系統而異。

### 案例三：MCP 工具市場（AI）

一間公司開發了一個「查詢庫存」的 MCP 伺服器。

只要支援 MCP 的 Agent 框架（如 Cursor、Claude Desktop 等）都可以直接使用這個工具。工具開發者只需開發一次，到處都能用。
...
MCP 是 tools / resources / prompts 的標準化通訊協定，不是 Agent 框架。

| | MCP | Agent Framework |
|---|---|---|
| 範圍 | 工具、資源、提示的標準化介面 | Agent 完整生命週期 |
...
綜合以上討論，使用工具時應遵循以下安全順序：

1. **最小權限**：只給 Agent 完成任務所需的最少工具與最少資料存取
2. **工具 allowlist**：只允許呼叫已被核准的工具清單，不在清單中的一律拒絕
3. **操作風險分級**：讀取類（低風險）、寫入類（中風險）、不可逆操作（高風險）
4. **高風險與不可逆操作的人類確認**：付款、刪除、退款、發送外部訊息等，應由使用者明確確認
5. **不信任外部工具回傳內容**：工具執行結果可能包含惡意內容，需獨立檢查
6. **日誌、稽核與可回復設計**：記錄所有工具呼叫，確保可回溯、可復原
...
| ★★★☆☆ | [OpenAI Agents SDK](https://github.com/openai/openai-agents-sdk) | OpenAI 官方 Agent 框架，示範 Tool Calling 標準實作 |
