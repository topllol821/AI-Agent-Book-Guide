...
這是本章最關鍵的問題。多 Agent 的價值來自幾個面向：

| 價值來源 | 說明 |
|---------|------|
| **新資訊** | Reviewer 執行測試後得到 Proposer 沒有的結果 |
| **平行探索** | 多個 Agent 同時嘗試不同方向，加快找到解答 |
| **Context 隔離** | 每個 Agent 專注在自己的上下文，避免單一 Agent Context 過載 |
| **專業分工** | 不同 Agent 使用不同工具或知識庫，各司其職 |
| **獨立驗證** | 不同的模型或不同的執行程式碼可交叉檢查結果 |

但應注意：**先確認一個單 Agent 或 Workflow 是否已足夠。** 多 Agent 不是為了比較酷，而是解決特定問題。
...
多 Agent 完成一個任務：
  → 假設 3 個 Agent，各迭代多次
  → 多次 LLM 呼叫
  → 每個 Agent 都有自己的 Context
  → 總成本可能大幅增加
```

確切的成本倍數取決於 Agent 數量、Context 大小、迭代輪數、工具呼叫與並行策略。**多 Agent 不是選項——它是一種投資。** 你必須以自己的任務評估成本與效益，確認收益大於成本時才應該用。
...
不對。多 Agent 的成本通常高於單 Agent（取決於 Agent 數量、迭代輪數與工具呼叫）。如果一個調校好的單 Agent 或 Workflow 就能解決問題，不需要用多 Agent。
...
> **多 Agent 的關鍵不只是「多」，而是平行探索、Context 隔離、專業分工或獨立驗證帶來了單 Agent 沒有的價值。如果只是讓同一個模型重複檢查自己的輸出，用多 Agent 只是浪費錢。**
...
1. **多 Agent 的關鍵判據**：協作是否因平行探索、Context 隔離、專業分工或獨立驗證帶來了額外價值
...
5. **多 Agent 的成本通常高於單 Agent**——不要為了用而用
