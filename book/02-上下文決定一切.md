...
> **補充說明（2026 年更新）：**
>
> 原書出版後，OpenAI 推出了 Persistent Contexts（2025 年底）和 Stateful API（2026 年初），Anthropic 也推出了 Project Context 的持久化機制。這些新功能讓部分上下文可以「黏在」專案或會話上，不需要每次重傳。
>
> 但核心觀念不變：**底層的 API 呼叫仍然是無狀態的**。這些新功能只是在框架層幫你自動管理上下文的載入和儲存，並沒有改變 LLM 本身的運作機制。
...
| 規則 | 說明 | 錯誤示範 | 正確做法 |
|------|------|----------|----------|
| 字尾不動 | System Prompt 定下來就不要改 | 在 System Prompt 裡加時間戳 | 時間戳作為新的 User Message 加在最後 |
| 動態放末尾 | 變動的資訊一律加在對話末尾 | 改變工具定義的順序 | 固定工具定義順序 |
| 用標準 API | 不要自己拼字串 | `f"User: {input}\nAssistant:"` | 用標準的 `messages` 陣列格式 |

**實際案例**：某團隊的客服 Agent 每天處理 10 萬次對話，一切正常。工程師為了讓 Agent「知道」當前時間，在 System Prompt 裡加了一行 `Current time: {{now}}`。第二天監控告警：所有對話延遲從 0.5 秒暴漲到 3-5 秒，帳單幾乎翻倍。原因？那一行時間戳讓 KV Cache 在每次請求都完全失效。
...
| ★★★☆☆ | [OpenClaw](https://github.com/anthropics/open-claw) | 開源 Agent，展示了漸進式披露的實作方式 |
...
4. **KV Cache 三條鐵律**：
   ...
   ```
   ① 字尾不動 — System Prompt 定下來就不要改
   ② 動態放末尾 — 時間戳、狀態資訊加在對話最後
   ③ 用標準 API — 不要自己拼 messages 字串
   ```
